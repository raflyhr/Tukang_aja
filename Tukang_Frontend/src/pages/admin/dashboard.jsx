...
const MOCK_ACTIVITIES = [
  {
    id: 1,
    text: "Pelanggan Rian Hidayat mendaftar ke platform",
    time: "13:15 WIB",
    icon: "group",
  ...
  {
    id: 2,
    text: "Mitra Tukang Joko Susilo berhasil diverifikasi",
    time: "12:40 WIB",
    icon: "verified",
  ...
  {
    id: 3,
    text: "Pembayaran TRX-9982 sebesar Rp 150.000 berhasil diterima",
    time: "11:30 WIB",
    icon: "check_circle",
    color: "text-green-400 bg-green-500/10",
  },
  {
    id: 4,
    text: "Laporan aduan baru masuk dari Dewi Lestari terkait keterlambatan",
    time: "10:15 WIB",
    icon: "warning",
  ...
  {
    id: 5,
    text: "Pendaftaran Tukang Ahmad ditolak karena KTP tidak terbaca",
    time: "09:45 WIB",
    icon: "block",
    color: "text-red-400 bg-red-500/10",
  },
  {
    id: 6,
    text: "Rating buruk (2.0) diterima oleh Agus Prasetyo",
    time: "Kemarin",
    icon: "star",
    color: "text-red-400 bg-red-500/10",
  },
...
const MOCK_TOP_TUKANG = [
  ...
  },
  {
    ...
    revenue: 8250000,
    online: false,
    avatar: "https://ui-avatars.com/api/?name=Hendra+Wijaya&background=random",
  },
];
...
const MOCK_NOTIFICATIONS = [
  {
    id: 1,
    text: "5 Mitra Tukang menunggu verifikasi",
    icon: "how_to_reg",
    path: "/admin/verifikasi",
  },
  {
    id: 2,
    text: "2 Laporan pengaduan baru masuk",
    icon: "report",
    path: "/admin/laporan",
  },
  {
    id: 3,
    text: "Pembayaran gagal dari TRX-9977 (Dewi Lestari)",
    icon: "error",
    path: "/admin/pembayaran",
  },
  {
    id: 4,
    text: "1 Akun terdeteksi rating bermasalah (< 3.0)",
    icon: "star_half",
    path: "/admin/rating",
  },
  {
    id: 5,
    text: "Pesanan baru TRX-9983 memerlukan alokasi",
...
function AdminDashboard() {
    ...
    try {
      ...
      if (response.data.status === "Sukses") {
        ...
        const fetchedVerifications = data.recentVerifications.map((v) => ({
          ...
          avatar: v.foto_profil
            ? v.foto_profil.startsWith("http")
              ? v.foto_profil
              : `${import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000"}/storage/${v.foto_profil}`
            : `https://ui-avatars.com/api/?name=${v.nama}&background=random`,
          status: "pending",
        }));
  ...
  const handleVerify = async (id) => {
    ...
    try {
      const response = await api.put(`/admin/verifikasi/${id}`, {
        action: "approve",
      });
      if (response.data.status === "Sukses") {
        setVerifications((prev) =>
  ...
  const searchResults = searchQuery
    ? {
        tukang: topTukang.filter(
          (t) =>
            t.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
            t.specialty.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
        pelanggan: topPelanggan.filter((p) =>
          p.name.toLowerCase().includes(searchQuery.toLowerCase()),
        ),
        aktivitas: recentActivities.filter((a) =>
  ...
  const showSearchResults =
    searchResults &&
    (searchResults.tukang.length > 0 ||
      searchResults.pelanggan.length > 0 ||
      searchResults.aktivitas.length > 0);
    ...
    <div className="bg-background text-on-surface min-h-screen selection:bg-secondary/30 selection:text-secondary font-sans relative overflow-x-hidden flex">
      ...
      >
        <div className="px-4 mb-8 flex justify-between items-center">
          ...
          <button
            className="lg:hidden text-on-surface-variant hover:text-on-surface cursor-pointer bg-transparent border-none"
            onClick={() => setIsSidebarOpen(false)}
          >
          ...
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-on-surface-variant/60 truncate uppercase tracking-wider">
                  {adminData.role}
                </p>
              </div>
            </div>
      ...
      <main className="flex-grow lg:ml-64 min-h-screen relative flex flex-col">
        ...
        <header className="fixed top-0 right-0 left-0 lg:left-64 z-40 flex justify-between items-center px-6 md:px-12 h-20 bg-surface/80 backdrop-blur-xl border-b border-surface-variant/20 transition-all duration-300">
          ...
          <div className="flex items-center gap-4 relative">
              ...
              <div className="absolute right-0 top-12 bg-surface-container-high border border-surface-variant/20 rounded-2xl w-80 py-3 shadow-2xl z-[60] overflow-hidden">
                <div className="px-4 pb-2 border-b border-surface-variant/15 flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="text-[10px] text-secondary font-bold bg-transparent border-none cursor-pointer"
                  >
                    Tutup
                  </button>
                </div>
            ...
            <button
              ...
              className="p-2 text-on-surface-variant cursor-pointer flex items-center justify-center bg-transparent border-none"
            >
        ...
        <div className="pt-28 pb-12 px-6 md:px-12 max-w-7xl w-full mx-auto space-y-8 flex-grow page-transition">
          ...
          <section className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="font-headline-xl text-headline-xl text-on-background mb-2 text-3xl font-bold mt-3">
              ...
              <p className="font-body-lg text-sm text-on-surface-variant/80 font-normal leading-relaxed">
                Pusat pemantauan aktivitas pelanggan, mitra tukang, pesanan, dan
                keuangan TukangAja.
              </p>
            </div>
          ...
          {isError ? (
            <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-12 text-center space-y-4">
              ...
              <p className="text-xs text-on-surface-variant max-w-md mx-auto leading-relaxed">
                Terjadi kegagalan koneksi ke API server Laravel backend
                (http://localhost:8000). Pastikan backend Anda sudah menyala.
              </p>
              <button
            ...
            <div className="bg-surface-container/40 border border-surface-variant/15 rounded-2xl p-12 text-center space-y-4">
              ...
              <p className="text-xs text-on-surface-variant max-w-md mx-auto leading-relaxed">
                Belum ada statistik atau pendaftaran masuk untuk ditampilkan
                saat ini.
              </p>
              <button
                onClick={fetchDashboardData}
            ...
            <div className="space-y-8 animate-pulse">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => (
                  <div
                    key={i}
                    className="bg-surface-container-high/40 h-28 rounded-2xl border border-surface-variant/10"
                  ></div>
                ))}
              </div>
            ...
            <section className="bg-surface-container/60 border border-surface-variant/20 rounded-2xl p-6 space-y-6">
              <div className="flex justify-between items-center border-b border-surface-variant/15 pb-3">
                ...
                <button
                  ...
                  className="text-xs text-on-surface-variant font-bold hover:underline bg-transparent border-none cursor-pointer"
                >
                  Clear
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">
                    Tukang ({searchResults.tukang.length})
                  </h4>
                  {searchResults.tukang.map((t, idx) => (
                    <div
                      key={idx}
                      className="bg-surface-container-high/40 p-3 rounded-xl border border-surface-variant/10 flex items-center gap-3"
                    >
                      <img
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">
                    Pelanggan ({searchResults.pelanggan.length})
                  </h4>
                ...
                <div className="space-y-3">
                  <h4 className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">
                    Log Aktivitas ({searchResults.aktivitas.length})
                  </h4>
                  {searchResults.aktivitas.map((a, idx) => (
                    <div
                      key={idx}
                      className="bg-surface-container-high/40 p-3 rounded-xl border border-surface-variant/10 text-xs"
                    >
                ...
                {[
                  {
                    ...
                    val: summary.pendingVerifications,
                    badge: `+${summary.pendingVerifications} Baru`,
                    color: "bg-secondary/15 text-secondary border-secondary/15",
                    icon: "pending_actions",
                    path: "/admin/verifikasi",
                  },
                  {
                    label: "Total Tukang",
                    val: summary.totalTukang,
                    badge: "+8% bln ini",
                    color: "bg-primary/15 text-primary border-primary/15",
                    icon: "engineering",
                    path: "/admin/data",
                  },
                  {
                    label: "Total Pelanggan",
                    val: summary.totalPelanggan,
                    badge: "+12% bln ini",
                  ...
                  },
                  {
                    ...
                    val: `Rp ${(summary.platformRevenue / 1000000).toFixed(1)}jt`,
                    badge: "+15% bln ini",
                    color:
                  ...
                  {
                    label: "Rating Bermasalah",
                    val: summary.problematicRatings,
                  ...
                  {
                    label: "Laporan Masuk",
                    val: summary.incomingReports,
                    badge: "Belum Selesai",
                    color: "bg-pink-500/10 text-pink-400 border-pink-500/15",
                    icon: "gavel",
                    path: "/admin/laporan",
                  },
                ].map((card, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div
                        className={`p-2.5 rounded-xl ${card.color} border shrink-0`}
                      >
                      ...
                      <span
                        className={`text-[9px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-full border ${card.color}`}
                      >
                        {card.badge}
                      </span>
                    </div>
                    <div className="mt-4">
                      <span className="block text-[10px] text-on-surface-variant/75 font-bold uppercase tracking-wider">
                        {card.label}
                      </span>
                      <h3 className="text-2xl font-black mt-1 text-on-surface">
                        {card.val}
                      </h3>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                  {[
                    ...
                    },
                    {
                      ...
                      desc: "Rekonsiliasi dana",
                      icon: "payments",
                      path: "/admin/pembayaran",
                      color: "text-yellow-400 bg-yellow-500/5",
                    },
                    {
                      title: "Monitoring Rating",
                      desc: "Cek kepuasan user",
                      icon: "star_rate",
                      path: "/admin/rating",
                    ...
                    >
                      <span
                        className={`material-symbols-outlined text-2xl p-2 rounded-lg w-10 h-10 flex items-center justify-center border border-surface-variant/10 ${act.color}`}
                      >
                        {act.icon}
                      </span>
                      <div className="mt-3">
                        ...
                        <p className="text-[9px] text-on-surface-variant mt-0.5 leading-tight">
                          {act.desc}
                        </p>
                      </div>
                    </div>
              ...
              <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-7 bg-surface-container/60 border border-surface-variant/15 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-on-surface-variant">
                      Pertumbuhan jumlah Tukang (Oranye) vs Pelanggan (Biru)
                    </p>
                  </div>
                ...
                <div className="lg:col-span-5 bg-surface-container/60 border border-surface-variant/15 rounded-2xl p-5 shadow-lg flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-on-surface-variant">
                      Rasio status pengerjaan pesanan
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-around gap-6 mt-6">
                    ...
                    <div className="text-[10px] space-y-2 font-bold uppercase tracking-wider text-on-surface-variant">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded bg-[#4ADE80]"></span>{" "}
                        Selesai (50%)
                      </div>
                      <div className="flex items-center gap-2">
                ...
                <div className="bg-surface-container/60 border border-surface-variant/15 rounded-2xl p-5 shadow-lg">
                  ...
                  <div className="space-y-3 font-medium">
                    {[
                      {
                        key: "Total Pelanggan",
                        val: MOCK_CUSTOMER_STATS.total,
                        icon: "people",
                      },
                      {
                        key: "Pelanggan Baru Minggu Ini",
                        val: MOCK_CUSTOMER_STATS.newThisWeek,
                        icon: "person_add",
                      },
                      {
                        key: "Aktif Hari Ini",
                        val: MOCK_CUSTOMER_STATS.activeToday,
                        icon: "bolt",
                      },
                      {
                        key: "Belum Pernah Memesan",
                        val: MOCK_CUSTOMER_STATS.neverOrdered,
                        icon: "person_off",
                      },
                      {
                        key: "Pelanggan Paling Aktif",
                        val: MOCK_CUSTOMER_STATS.mostActive,
                        icon: "military_tech",
                      },
                    ].map((row, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-center bg-surface-container-high/40 p-3 rounded-xl border border-surface-variant/10 text-xs"
                      >
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        },
                        {
                          ...
                          current: MOCK_ORDER_STATS.processing,
                          total: 1303,
                          percentage: 12,
                          color: "bg-blue-500",
                        },
                        {
                          ...
                          current: MOCK_ORDER_STATS.pendingPayment,
                          total: 1303,
                          percentage: 6,
                          color: "bg-purple-500",
                        },
                        {
                          label: "Selesai",
                          current: MOCK_ORDER_STATS.completed,
                          total: 1303,
                          percentage: 85,
                          color: "bg-green-500",
                        },
                        {
                          label: "Dibatalkan",
                          current: MOCK_ORDER_STATS.cancelled,
                          total: 1303,
                          percentage: 4,
                          color: "bg-red-500",
                        },
                ...
                <div className="bg-surface-container/60 border border-surface-variant/15 rounded-2xl p-5 shadow-lg">
                  ...
                  <div className="space-y-3 font-medium">
                    {topTukang.map((item, idx) => (
                      <div
                        key={idx}
                        className="bg-surface-container-high/40 p-3 rounded-xl border border-surface-variant/10 flex items-center gap-3 text-xs justify-between"
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          <div className="relative shrink-0">
                            ...
                            />
                            {item.online && (
                              <span className="absolute -bottom-1 -right-1 w-2.5 h-2.5 bg-green-500 border-2 border-surface-container-high rounded-full"></span>
                            )}
                          </div>
                          <div className="min-w-0">
                            <h5 className="font-bold text-on-surface truncate">
                              {item.name}
                            </h5>
                            <p className="text-[10px] text-on-surface-variant truncate">
                              {item.specialty} • {item.orders} order
                            </p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          ...
                          <span className="text-[9px] text-on-surface-variant/80 font-bold block mt-0.5">
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-on-surface-variant truncate">
                              {item.orders}x Order • Aktif: {item.lastActive}
                            </p>
                          </div>
                        </div>
                        <div className="text-right shrink-0">
                          <span className="font-bold text-on-surface text-[11px]">
                            Rp {(item.spending / 1000000).toFixed(1)}jt
                          </span>
                          <span className="text-[8px] uppercase tracking-wider text-green-400 font-bold block mt-1">
                            Loyal Member
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="space-y-3">
                    {verifications.length === 0 ? (
                      <div className="bg-surface-container/50 p-8 rounded-2xl text-center text-on-surface-variant/65 border border-surface-variant/10">
                        <span className="material-symbols-outlined text-3xl mb-1.5 text-secondary/45">
                          verified_user
                        </span>
                        <p className="text-xs font-semibold">
                          Semua pengajuan tukang telah selesai diverifikasi!
                        </p>
                      </div>
                    ) : (
                        ...
                        >
                          ...
                          <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3 w-full text-xs">
                            <div className="flex flex-col justify-center">
                              ...
                              <p className="text-secondary text-[10px] font-bold mt-0.5">
                                {item.type}
                              </p>
                            </div>
                    ...
                    <div className="space-y-4 relative pl-4 border-l border-surface-variant/20 font-medium">
                      {recentActivities.map((act) => (
                        <div key={act.id} className="relative text-xs">
                          {/* Dot marker */}
                          <div
                            className={`absolute -left-[22px] top-1 w-2.5 h-2.5 rounded-full border-2 border-surface-container ${act.color.split(" ")[0].replace("text-", "bg-")}`}
                          ></div>
                          <div>
